...
namespace EdgeSLAM {
	KeyFrameDB::KeyFrameDB(DBoW3::Vocabulary *voc) :
		mpVoc(voc)
	{
		mvInvertedFile.resize(voc->size());
	}
	/*KeyFrameDB::KeyFrameDB(ORBVocabulary *voc) :
		mpVoc(voc)
	{
		mvInvertedFile.resize(voc->size());
	}*/
	...
	void KeyFrameDB::erase(KeyFrame* pKF)
	{
		std::unique_lock<std::mutex> lock(mMutex);
	...
	std::vector<KeyFrame*> KeyFrameDB::DetectRelocalizationCandidates(Frame *F)
	{
		...
		// Search all keyframes that share a word with current frame
		{
			...
			for (auto vit = F->mBowVec.begin(), vend = F->mBowVec.end(); vit != vend; vit++)
			{
				...
				{
					KeyFrame* pKFi = *lit;
					if (pKFi->mnRelocQuery != F->mnFrameID)
					{
						pKFi->mnRelocWords = 0;
						pKFi->mnRelocQuery = F->mnFrameID;
						lKFsSharingWords.push_back(pKFi);
					}
					pKFi->mnRelocWords++;
				}
		...
		for (auto it = lScoreAndMatch.begin(), itend = lScoreAndMatch.end(); it != itend; it++)
		{
			...
			{
				KeyFrame* pKF2 = *vit;
				if (pKF2->mnRelocQuery != F->mnFrameID)
					continue;
