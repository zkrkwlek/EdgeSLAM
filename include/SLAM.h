...
namespace EdgeSLAM {
	...
	public:
		...
		void CreateUser(std::string _user, std::string _map, int _w, int _h, float _fx, float _fy, float _cx, float _cy, float _d1, float _d2, float _d3, float _d4, float _d5, int quality, int nskip, bool _b, bool _bTracking = false, bool _bIMU = false, bool _bGBA = false, bool _bReset = false, bool _bsave = false, bool _basync = false);
