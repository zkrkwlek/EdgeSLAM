...
namespace EdgeSLAM {
	...
	public:
		...
		static void TrackSimulation(ThreadPool::ThreadPool* pool, SLAM* system, int id, std::string user, Frame* frame, const cv::Mat& img, double ts);
		static void Track(ThreadPool::ThreadPool* pool, SLAM* system, int id, std::string user, Frame* frame, const cv::Mat& img, double ts);
